...
export default function FoodsCatalogPage() {
  ...
  const itemList = {
    ...
    name: "Harobal Foods commodity catalogue",
    numberOfItems: families.length,
    itemListElement: families.map((family, index) => ({
      "@type": "ListItem",
      position: index + 1,
      name: family.title,
      url: absoluteUrl(`/catalog/${family.variants[0]?.slug}`),
    })),
  };
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden border-b border-white/10 bg-brand-ink text-white">
        <div
          className="absolute inset-0 bg-[linear-gradient(90deg,rgba(7,25,20,.96)_0%,rgba(7,25,20,.82)_45%,rgba(7,25,20,.28)_100%),url('/media/harvest-meridian/foods-export-hero.webp')] bg-cover bg-center"
          aria-hidden
        />
        <div className="container-shell relative py-8 sm:py-12">
          <div className="max-w-3xl">
            ...
            <dl className="mt-5 grid grid-cols-3 gap-3 border-t border-white/15 pt-4 sm:mt-7 sm:max-w-xl sm:gap-8 sm:pt-5">
              <div>
                <dt className="text-[8px] font-bold uppercase tracking-[.12em] text-white/55 sm:text-[10px] sm:tracking-[.16em]">
                  Product families
                </dt>
                <dd className="mt-1 text-lg font-bold sm:text-xl">{families.length}</dd>
              </div>
              <div>
                <dt className="text-[8px] font-bold uppercase tracking-[.12em] text-white/55 sm:text-[10px] sm:tracking-[.16em]">
                  Exact specifications
                </dt>
                <dd className="mt-1 text-lg font-bold sm:text-xl">{specificationCount}</dd>
              </div>
              <div>
                <dt className="text-[8px] font-bold uppercase tracking-[.12em] text-white/55 sm:text-[10px] sm:tracking-[.16em]">
                  Trade categories
                </dt>
                <dd className="mt-1 text-lg font-bold sm:text-xl">{categoryCount}</dd>
              </div>
            </dl>
          </div>
        </div>
