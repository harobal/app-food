...
function FilterGroup({
  ...
  // Auto-expand if options inside this group are selected
  useEffect(() => {
  ...
  return (
    <div className="border-b border-border py-3.5 last:border-b-0">
      <button
        ...
        className="flex w-full cursor-pointer items-center justify-between gap-3 text-left text-sm font-bold transition-colors hover:text-primary"
            ...
            options.map((option) => {
              ...
              return (
                <label
                  key={option}
                  className={cn(
                    "flex cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-xs sm:text-sm transition-colors",
                    checked
                ...
                >
                  ...
                  <span className="text-[11px] tabular-nums text-muted-foreground/75 font-mono">{count}</span>
                </label>
              );
            })
          )}
  ...
  return (
    <div className="fixed inset-0 z-[70] lg:hidden">
      ...
      <div
        ...
        className="absolute inset-y-0 right-0 flex w-[92%] max-w-md flex-col bg-background shadow-2xl"
      >
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[.18em] text-primary">Refine results</p>
            <h2 className="mt-1 text-xl font-bold">Filters</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="grid grid-cols-[auto_1fr] gap-3 border-t border-border bg-background p-4">
          ...
          <Button type="button" onClick={onClose}>
            Show {count} {count === 1 ? "family" : "families"}
          </Button>
        </div>
      </div>
    </div>
...
export function FoodsCatalogClient({ families: allFamilies }: { families: FoodsCatalogFamily[] }) {
  ...
  const filters = useMemo(
    () => sanitizeCatalogFilters(parseCatalogFilters(new URLSearchParams(searchParams.toString())), options),
  ...
  const update = useCallback(
    <K extends keyof FoodsCatalogFilters>(key: K, value: FoodsCatalogFilters[K]) =>
      navigate({ ...filters, [key]: value, page: key === "page" ? Number(value) : 1 }),
    [filters, navigate],
  );
  ...
  const optionCounts = useMemo(() => {
    ...
    for (const family of allFamilies) {
      ...
      for (const value of family.certificationsAvailable)
        counts.certifications[value] = (counts.certifications[value] ?? 0) + 1;
    }
    return counts;
  }, [allFamilies]);
  ...
  const renderFilterControls = () => (
    <div>
      <FilterGroup
        ...
        counts={optionCounts.categories}
        initiallyOpen
        onToggle={(value) => update("categories", toggle(filters.categories, value))}
      />
      <FilterGroup
        name="form"
        ...
        counts={optionCounts.forms}
        initiallyOpen
        onToggle={(value) => update("forms", toggle(filters.forms, value))}
      />
      <FilterGroup
        name="origin"
        label="Origin in India"
        options={options.origins}
        selected={filters.origins}
        counts={optionCounts.origins}
        initiallyOpen
        onToggle={(value) => update("origins", toggle(filters.origins, value))}
      />
      <FilterGroup
        name="certification"
        label="Certification availability"
        options={options.certifications}
        selected={filters.certifications}
        counts={optionCounts.certifications}
        onToggle={(value) => update("certifications", toggle(filters.certifications, value))}
      />
    </div>
  );
  ...
  return (
    <>
        ...
        <div className="grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center">
          <label className="flex h-12 items-center gap-3 rounded-xl border border-input bg-background px-4 focus-within:border-primary/50 focus-within:ring-3 focus-within:ring-ring/20">
            ...
            <input
              ...
              placeholder="Search turmeric, basmati, powder, Gujarat…"
          ...
          <Button type="button" variant="outline" className="h-12 lg:hidden" onClick={() => setFilterOpen(true)}>
          ...
          <Button asChild variant="outline" className="h-12 border-primary/20 bg-primary/5 text-primary">
            <FoodsLink href="/rfq">
              <PackageCheck className="size-4" /> RFQ list {quoteCount ? `(${quoteCount})` : ""}
            </FoodsLink>
          </Button>
        </div>
        <div className="border-t border-border bg-muted/25 px-4 py-3">
          ...
          >
            ...
            {options.categories.map((category) => {
              const active = filters.categories.length === 1 && filters.categories[0] === category;
              return (
                <button
                  key={category}
                  type="button"
                  onClick={() => update("categories", active ? [] : [category])}
                  className={cn(
                ...
                >
                  {category} <span className="opacity-65">{optionCounts.categories[category]}</span>
                </button>
              );
            })}
          </div>
      ...
      <div className="grid gap-7 lg:grid-cols-[18rem_minmax(0,1fr)]">
        <aside className="sticky top-28 hidden h-[calc(100dvh-8rem)] min-h-[22rem] self-start lg:block">
          <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            <div className="flex shrink-0 items-center justify-between border-b border-border bg-brand-ink px-5 py-4 text-white">
              <span className="flex items-center gap-2">
                <Filter className="size-4 text-brand-gold" />
                <span className="text-sm font-bold">Refine results</span>
              </span>
              {hasFilters ? (
                <button type="button" onClick={clear} className="text-xs font-bold text-brand-gold hover:text-white">
                  Clear all
                </button>
              ) : null}
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 [scrollbar-color:var(--brand-primary)_transparent] [scrollbar-width:thin]">
              {renderFilterControls()}
            </div>
            <div className="shrink-0 border-t border-border bg-muted/25 p-4 text-xs leading-5 text-muted-foreground">
              Filters stay available while this panel scrolls independently.
            </div>
          </div>
        </aside>
        ...
        <section ref={resultsRef} className="min-w-0 scroll-mt-28" aria-busy={isPending}>
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
            <div>
              <p className="text-xl font-bold" role="status" aria-live="polite">
                {families.length} {families.length === 1 ? "product family" : "product families"}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {matchingVariants} exact specifications available for selection.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <label className="flex h-10 items-center gap-2 rounded-lg border border-border bg-card px-3 text-xs font-bold text-muted-foreground">
                Sort{" "}
                <select
                  value={filters.sort}
                  onChange={(event) => update("sort", event.target.value as FoodsCatalogFilters["sort"])}
          ...
          {filters.query || chips.length ? (
            <div className="flex flex-wrap items-center gap-2 border-b border-border py-4" aria-label="Applied filters">
              {filters.query ? (
                <button
                  type="button"
                  onClick={() => setSearchValue("")}
                  className="inline-flex items-center gap-1 rounded-full border border-primary/20 bg-primary/7 px-3 py-1.5 text-xs font-semibold text-primary"
                >
                  Search: “{filters.query}” <X className="size-3.5" />
                </button>
              ) : null}
              {chips.map((chip) => (
                <button
                  ...
                  type="button"
                  onClick={() => update(chip.group, toggle(filters[chip.group], chip.value))}
                  className="inline-flex items-center gap-1 rounded-full border border-primary/20 bg-primary/7 px-3 py-1.5 text-xs font-semibold text-primary"
                >
                  {chip.value}
          ...
          <div className="pt-5">
            {visibleFamilies.length ? (
              <>
                <div
                  className={cn(
                    ...
                    view === "grid" ? "md:grid-cols-2 2xl:grid-cols-3" : "grid-cols-1",
                    isPending && "opacity-55",
                  )}
                >
                  {visibleFamilies.map((family) => (
                    <CatalogFamilyCard key={family.key} family={family} view={view} />
                  ))}
                </div>
                {pageCount > 1 ? (
                  <nav
                  ...
                  >
                    <p className="text-xs font-medium text-muted-foreground">
                      Showing {pageStart + 1}–{Math.min(pageStart + PAGE_SIZE, families.length)} of {families.length}{" "}
                      product families
                    </p>
              ...
              <div className="relative overflow-hidden rounded-3xl border border-dashed border-border bg-card px-6 py-16 text-center">
                <div className="absolute inset-x-0 top-0 h-1 bg-[linear-gradient(90deg,var(--brand-primary),var(--brand-gold))]" />
                <Search className="mx-auto size-8 text-primary" />
                <p className="mt-5 text-2xl font-bold">No products match this brief</p>
                <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">
                  Remove a filter or broaden the search. If the required item is not listed, the sourcing desk can still
                  review it through a custom RFQ.
                </p>
                <div className="mt-6 flex flex-wrap justify-center gap-3">
                  ...
                  <Button asChild>
                    <FoodsLink href="/contact">Ask the sourcing desk</FoodsLink>
                  </Button>
                </div>
              </div>
            )}
      ...
      {quoteCount > 0 ? (
        <div className="sticky bottom-4 z-30 mx-auto mt-10 hidden max-w-xl items-center justify-between gap-4 rounded-2xl border border-primary/20 bg-brand-ink px-5 py-4 text-white shadow-2xl sm:flex">
          <div>
            <p className="text-sm font-bold">
              {quoteCount} {quoteCount === 1 ? "specification" : "specifications"} in your RFQ
            </p>
            <p className="text-xs text-white/65">Review quantities, notes, and shipment details.</p>
          </div>
          <Button asChild size="sm" className="shrink-0 bg-accent text-accent-foreground hover:bg-[#d9a748]">
            <FoodsLink href="/rfq">Review RFQ</FoodsLink>
          </Button>
        </div>
      ) : null}
